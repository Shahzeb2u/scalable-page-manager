 ...
 * Plugin URI:        https://github.com/sr-dev/scalable-page-manager
 ...
 * Text Domain:       scalable-page-manager
 *
...
/*
 * Justification for the disabled DB sniffs above:
 *
 * This plugin operates directly on core tables (wp_posts, wp_postmeta) by
 * design: it is a high-volume page manager that must use keyset pagination,
 ...
 * (a) fixed table names from $wpdb ($wpdb->posts / $wpdb->postmeta), and
 * (b) machine-generated IN() placeholder lists of the form %d,%d,... whose
 * count matches the bound parameter array. All user-supplied values are
 * passed through $wpdb->prepare(). No user input is ever concatenated into
 * SQL. Hot read paths (stats, tree, parents, duplicates) are cached with the
 * Transients API; the remaining calls are per-request admin reads or one-off
 * idempotent DDL guarded by activation. The SchemaChange calls build indexes
 * lazily, one per request, to avoid gateway timeouts on large tables.
 */

define( 'SPM_VERSION', '1.5.0' );
...
function spm_register_menu() {
	add_menu_page(
		__( 'Pages (Scalable)', 'scalable-page-manager' ),
		__( 'Pages (Scalable)', 'scalable-page-manager' ),
		'edit_pages',
		SPM_SLUG,
...
function spm_render_admin_page() {
	if ( ! current_user_can( 'edit_pages' ) ) {
		wp_die( esc_html__( 'You do not have permission to manage pages.', 'scalable-page-manager' ) );
	}
	...
	?>
	<div class="wrap">
		<h1 class="wp-heading-inline"><?php esc_html_e( 'Pages (Scalable)', 'scalable-page-manager' ); ?></h1>
		<a href="<?php echo esc_url( $boot['newUrl'] ); ?>" class="page-title-action"><?php esc_html_e( 'Add New', 'scalable-page-manager' ); ?></a>
		<hr class="wp-header-end">
		<?php if ( 'done' !== $boot['indexStatus'] ) : ?>
			<div class="notice notice-info" style="margin:10px 0">
				<p><strong><?php esc_html_e( 'Optimizing search indexes in the background…', 'scalable-page-manager' ); ?></strong>
				<?php esc_html_e( 'The page manager is fully usable now. Title/content search runs in basic mode until indexing finishes (this can take a few minutes on large sites). You can speed it up by running', 'scalable-page-manager' ); ?>
				<code>wp spm build-indexes</code> <?php esc_html_e( 'via WP-CLI.', 'scalable-page-manager' ); ?></p>
			</div>
		<?php endif; ?>
		<div id="spm-app"><p class="spm-muted"><?php esc_html_e( 'Loading page manager…', 'scalable-page-manager' ); ?></p></div>
	</div>
...
function spm_get_page_templates() {
	...
	$out   = array( array( 'value' => '', 'label' => __( 'Any template', 'scalable-page-manager' ) ) );
...
function spm_get_page_authors() {
	...
	$out = array( array( 'value' => 0, 'label' => __( 'Any author', 'scalable-page-manager' ) ) );
	foreach ( $users as $u ) {
...
function spm_shape_row( array $r ) {
	...
	return array(
		'id'            => $id,
		'title'         => ( '' !== $r['post_title'] ) ? $r['post_title'] : __( '(no title)', 'scalable-page-manager' ),
		'slug'          => $r['post_name'],
...
function spm_stream_csv( array $rows ) {
	...
	$out = fopen( 'php://output', 'w' ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fopen -- writing a streamed CSV download to the HTTP response body.

	// UTF-8 BOM so Excel renders accented characters correctly on open.
	fwrite( $out, "\xEF\xBB\xBF" ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fwrite -- streaming CSV to php://output for download; WP_Filesystem cannot write to the response body.
	...
	fclose( $out ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_system_operations_fclose -- closing the php://output stream opened above.
	exit;
}
...
function spm_rest_edit_data( WP_REST_Request $req ) {
	...
	if ( ! current_user_can( 'edit_post', $id ) ) {
		return new WP_Error( 'spm_forbidden', __( 'Not allowed.', 'scalable-page-manager' ), array( 'status' => 403 ) );
	}
	$post = get_post( $id );
	if ( ! $post ) {
		return new WP_Error( 'spm_not_found', __( 'Page not found.', 'scalable-page-manager' ), array( 'status' => 404 ) );
	}
...
function spm_rest_quick_edit( WP_REST_Request $req ) {
	$id = (int) $req['id'];
	if ( ! current_user_can( 'edit_post', $id ) ) {
		return new WP_Error( 'spm_forbidden', __( 'Not allowed to edit this page.', 'scalable-page-manager' ), array( 'status' => 403 ) );
	}
...
function spm_rest_action( WP_REST_Request $req ) {
	...
	if ( ! $id || ! get_post( $id ) ) {
		return new WP_Error( 'spm_bad_request', __( 'Invalid page.', 'scalable-page-manager' ), array( 'status' => 400 ) );
	}
	...
	return new WP_Error( 'spm_unknown_action', __( 'Unknown action.', 'scalable-page-manager' ), array( 'status' => 400 ) );
}

function spm_duplicate_page( $id ) {
	$src = get_post( $id );
	if ( ! $src ) {
		return new WP_Error( 'spm_no_src', __( 'Source page not found.', 'scalable-page-manager' ) );
	}
	$new_id = wp_insert_post( array(
		'post_type'      => 'page',
		'post_status'    => 'draft',
		'post_title'     => $src->post_title . ' (' . __( 'copy', 'scalable-page-manager' ) . ')',
...
function spm_register_user_journey_tax() {
	register_taxonomy( SPM_USER_JOURNEY_TAX, 'page', array(
		'label'             => __( 'User Journey', 'scalable-page-manager' ),
		'labels'            => array(
			'name'          => __( 'User Journey', 'scalable-page-manager' ),
			'singular_name' => __( 'User Journey', 'scalable-page-manager' ),
			'menu_name'     => __( 'User Journey', 'scalable-page-manager' ),
		),
		'public'            => false,
