 ...
 * Text Domain:       pagescale
 *
 * Single-file plugin. No build step required: the admin UI is vanilla JS,
 * served inline and powered by a custom REST namespace (pagescale/v1).
 *
 * ============================================================================
 * SCALING NOTES
 * ----------------------------------------------------------------------------
 ...
 * wp_posts / wp_postmeta tables directly. It must use keyset pagination,
 * FULLTEXT MATCH..AGAINST and GROUP BY queries that WP_Query cannot express
 * efficiently at 100k+ rows. In every query the ONLY interpolated tokens are
 * fixed core table names from $wpdb ( $wpdb->posts / $wpdb->postmeta ) and
 * machine-generated %d / %s placeholder lists whose count matches the bound
 * parameter array. All user-supplied values pass through $wpdb->prepare().
 * Hot read paths (stats, parents) are cached with the Transients API. The
 * following sniffs are therefore acknowledged and handled at the query level:
 */
// phpcs:disable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare

/* ===========================================================================
 * ACTIVATION : create composite + FULLTEXT indexes (guarded, idempotent)
 * ======================================================================== */

register_activation_hook( __FILE__, 'pgscl_activate' );
 ...
 * and one-at-a-time via pgscl_maybe_build_indexes() on later admin loads, and
 * the plugin works fully (just unindexed) until they exist.
 */
function pgscl_activate() {
	if ( false === get_option( 'pgscl_index_status', false ) ) {
		add_option( 'pgscl_index_status', 'pending' ); // pending | building | done
	}
	update_option( 'pgscl_db_version', PGSCL_DB_VERSION );
}

register_deactivation_hook( __FILE__, 'pgscl_deactivate' );
function pgscl_deactivate() {
	$ts = wp_next_scheduled( 'pgscl_build_indexes_event' );
	if ( $ts ) {
		wp_unschedule_event( $ts, 'pgscl_build_indexes_event' );
	}
}

/** True if a named index currently exists on a table (cached per request). */
function pgscl_index_exists( $table, $name ) {
	static $cache = array();
	$ck = $table . '|' . $name;
	if ( isset( $cache[ $ck ] ) ) {
		return $cache[ $ck ];
 ...
 */
function pgscl_index_definitions() {
	global $wpdb;
	$posts = $wpdb->posts;
	$pm    = $wpdb->postmeta;
	return array(
		array( $posts, 'pgscl_parent_order',  "ADD INDEX pgscl_parent_order (post_parent, post_type, menu_order, ID)" ),
		array( $posts, 'pgscl_modified',      "ADD INDEX pgscl_modified (post_type, post_status, post_modified, ID)" ),
		array( $posts, 'pgscl_author_status', "ADD INDEX pgscl_author_status (post_author, post_type, post_status, post_date)" ),
		array( $posts, 'pgscl_title_ft',      "ADD FULLTEXT INDEX pgscl_title_ft (post_title)" ),
		array( $posts, 'pgscl_content_ft',    "ADD FULLTEXT INDEX pgscl_content_ft (post_content)" ),
		array( $pm,    'pgscl_meta_value_ft', "ADD FULLTEXT INDEX pgscl_meta_value_ft (meta_value)" ),
	);
}
 ...
 */
function pgscl_maybe_build_indexes() {
	$status = get_option( 'pgscl_index_status', 'pending' );
	if ( 'done' === $status ) {
		return false;
	}
	...
	$last = (int) get_option( 'pgscl_index_last_attempt', 0 );
	if ( time() - $last < 30 ) {
		return true;
	}
	update_option( 'pgscl_index_last_attempt', time() );

	global $wpdb;
	$built_any = false;

	foreach ( pgscl_index_definitions() as $def ) {
	...
	if ( ! $built_any ) {
		...
		update_option( 'pgscl_index_status', 'done' );
		return false;
	}

	update_option( 'pgscl_index_status', 'building' );
	return true;
}
...
add_action( 'pgscl_build_indexes_event', 'pgscl_cron_build_indexes' );
function pgscl_cron_build_indexes() {
	// In cron there is no gateway timeout; still build one-at-a-time and
	// re-schedule until done so each event stays short.
	if ( pgscl_maybe_build_indexes() ) {
		if ( ! wp_next_scheduled( 'pgscl_build_indexes_event' ) ) {
			wp_schedule_single_event( time() + 60, 'pgscl_build_indexes_event' );
		}
	}
}
...
// pgscl_maybe_build_indexes() does at most one index and is throttled to 30s.
add_action( 'admin_init', 'pgscl_schedule_index_build' );
function pgscl_schedule_index_build() {
	if ( 'done' === get_option( 'pgscl_index_status', 'pending' ) ) {
		return;
	}
	if ( ! wp_next_scheduled( 'pgscl_build_indexes_event' ) ) {
		wp_schedule_single_event( time() + 5, 'pgscl_build_indexes_event' );
	}
}

// WP-CLI: build all indexes immediately (no timeout on CLI). `wp pgscl build-indexes`
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	WP_CLI::add_command( 'pgscl build-indexes', function () {
		$guard = 0;
		while ( pgscl_maybe_build_indexes() && $guard < 50 ) {
			// Bypass the 30s throttle on CLI.
			update_option( 'pgscl_index_last_attempt', 0 );
			$guard++;
		}
		WP_CLI::success( 'Scalable Page Manager: indexes built (status: ' . get_option( 'pgscl_index_status' ) . ').' );
	} );
}
...
/**
 * Enqueue admin assets (styles + app JS + boot data) on our screen only.
 * Replaces the previous inline <script>/<style> output for WP.org compliance.
 */
add_action( 'admin_enqueue_scripts', 'pgscl_enqueue_assets' );
function pgscl_enqueue_assets( $hook ) {
	// Our submenu screen hook ends with the plugin slug.
	if ( strpos( (string) $hook, PGSCL_SLUG ) === false ) {
		return;
	}

	// Styles: registered with no external file, CSS added inline.
	wp_register_style( 'pgscl-admin', false, array(), PGSCL_VERSION );
	wp_enqueue_style( 'pgscl-admin' );
	wp_add_inline_style( 'pgscl-admin', pgscl_get_styles() );

	// Script: registered with no external file, JS added inline (footer).
	wp_register_script( 'pgscl-admin', false, array(), PGSCL_VERSION, true );
	wp_enqueue_script( 'pgscl-admin' );
	wp_add_inline_script( 'pgscl-admin', 'window.PGSCL_BOOT = ' . wp_json_encode( pgscl_get_boot_data() ) . ';', 'before' );
	wp_add_inline_script( 'pgscl-admin', pgscl_get_app_js() );
}
	...
	<div class="wrap">
		...
		<?php if ( 'done' !== $boot['indexStatus'] ) : ?>
			<div class="notice notice-info" style="margin:10px 0">
				<p><strong><?php esc_html_e( 'Optimizing search indexes in the background…', 'pagescale' ); ?></strong>
				<?php esc_html_e( 'The page manager is fully usable now. Title/content search runs in basic mode until indexing finishes (this can take a few minutes on large sites). You can speed it up by running', 'pagescale' ); ?>
				<code>wp pgscl build-indexes</code> <?php esc_html_e( 'via WP-CLI.', 'pagescale' ); ?></p>
			</div>
		<?php endif; ?>
		<div id="spm-app"><p class="spm-muted"><?php esc_html_e( 'Loading page manager…', 'pagescale' ); ?></p></div>
	</div>
	<?php
}

function pgscl_elementor_active() {
	return did_action( 'elementor/loaded' ) || defined( 'ELEMENTOR_VERSION' );
}

function pgscl_get_page_templates() {
	$theme = wp_get_theme();
	$tpls  = $theme ? $theme->get_page_templates() : array();
	$out   = array( array( 'value' => '', 'label' => __( 'Any template', 'pagescale' ) ) );
...
function pgscl_get_page_authors() {
	...
	$out = array( array( 'value' => 0, 'label' => __( 'Any author', 'pagescale' ) ) );
	foreach ( $users as $u ) {
 ...
 *
 * Detection order:
 * 1. 'pgscl_template_taxonomy' filter (explicit override — most reliable)
...
function pgscl_get_template_taxonomy_label() {
	$tax = pgscl_get_template_taxonomy_name();
 ...
 */
function pgscl_get_template_taxonomy_terms() {
	$tax = pgscl_get_template_taxonomy_name();
...
/**
 * Remove private pages the current user is not allowed to read from a raw
 * result set (rows are associative arrays containing post_status/post_author).
 * Endpoints that build their own SQL (tree, search, duplicates) run their
 * results through this so private pages never leak to users lacking the
 * read_private_pages capability.
 */
function pgscl_filter_readable( array $rows ) {
	if ( current_user_can( 'read_private_pages' ) ) {
		return $rows;
	}
	$uid = get_current_user_id();
	return array_values( array_filter( $rows, function ( $r ) use ( $uid ) {
		$status = isset( $r['post_status'] ) ? $r['post_status'] : '';
		$author = isset( $r['post_author'] ) ? (int) $r['post_author'] : 0;
		return 'private' !== $status || $author === $uid;
	} ) );
}

function pgscl_register_routes() {
	$perm = 'pgscl_can_edit_pages';

	register_rest_route( 'pagescale/v1', '/pages', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_list',
		'permission_callback' => $perm,
	) );

	register_rest_route( 'pagescale/v1', '/tree/(?P<parent>\d+)', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_tree',
		'permission_callback' => $perm,
		'args'                => array( 'parent' => array( 'sanitize_callback' => 'absint' ) ),
	) );

	register_rest_route( 'pagescale/v1', '/search', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_search',
		'permission_callback' => $perm,
	) );

	register_rest_route( 'pagescale/v1', '/stats', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_stats',
		'permission_callback' => $perm,
	) );
	...
	register_rest_route( 'pagescale/v1', '/page/(?P<id>\d+)/edit', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_edit_data',
		'permission_callback' => $perm,
		'args'                => array( 'id' => array( 'sanitize_callback' => 'absint' ) ),
	) );

	register_rest_route( 'pagescale/v1', '/action', array(
		'methods'             => 'POST',
		'callback'            => 'pgscl_rest_action',
		'permission_callback' => $perm,
	) );

	register_rest_route( 'pagescale/v1', '/duplicates', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_duplicates',
		'permission_callback' => $perm,
	) );

	register_rest_route( 'pagescale/v1', '/parents', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_parents',
		'permission_callback' => $perm,
	) );

	// CSV export of the currently-filtered (or currently-searched) view.
	register_rest_route( 'pagescale/v1', '/export', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_export',
		'permission_callback' => $perm,
	) );

	// FEATURE 2: custom 4-tier top-level default view (empty search).
	register_rest_route( 'pagescale/v1', '/default-view', array(
		'methods'             => 'GET',
		'callback'            => 'pgscl_rest_default_view',
		'permission_callback' => $perm,
	) );
}
...
const PGSCL_STATUSES = array( 'publish', 'draft', 'pending', 'private', 'future', 'trash' );
...
function pgscl_build_filters( WP_REST_Request $req ) {
	...
	$status = array_values( array_intersect( $status, PGSCL_STATUSES ) );
	if ( $status ) {
	...
	// Private pages must not be exposed to users who cannot read them. Without
	// the read_private_pages capability, only show a private page to its author.
	if ( ! current_user_can( 'read_private_pages' ) ) {
		$where[]  = "( p.post_status <> 'private' OR p.post_author = %d )";
	...
	if ( $tmpl_tax_hint && $tmpl_tax_hint !== '__detect__' && taxonomy_exists( $tmpl_tax_hint ) ) {
		...
		$tmpl_tax = pgscl_get_template_taxonomy_name();
	}
...
function pgscl_shape_row( array $r ) {
	$id = (int) $r['ID'];
	...
	return array(
		'id'            => $id,
		'title'         => ( '' !== $r['post_title'] ) ? $r['post_title'] : __( '(no title)', 'pagescale' ),
		'slug'          => $r['post_name'],
...
function pgscl_rest_list( WP_REST_Request $req ) {
	...
	$rows = $wpdb->get_results( $wpdb->prepare( $sql, $params ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $sql is a static string with %d/%s placeholders; all values bound via prepare().
	...
	return rest_ensure_response( array(
		'rows'        => array_map( 'pgscl_shape_row', $rows ),
		'next_cursor' => $next,
	) );
}
 ...
 *
 *   1. FILTER MODE (default): reuses pgscl_build_filters() — the same WHERE
 ...
 *      (PGSCL_EXPORT_MAX) protects against runaway memory on huge sites.
 *
 *   2. ID MODE: when the view is a text search or duplicate scan, those
 *      results don't come from pgscl_build_filters(), so the client passes the
...
if ( ! defined( 'PGSCL_EXPORT_MAX' ) ) {
	// Safety ceiling on rows per export. Filter with 'pgscl_export_max' if needed.
	define( 'PGSCL_EXPORT_MAX', 50000 );
}
 ...
 */
function pgscl_export_columns( array $shaped ) {
	return array(
		'ID'             => $shaped['id'],
		'Title'          => $shaped['title'],
		'Slug'           => $shaped['slug'],
		...
		'Author'         => pgscl_export_author_name( $shaped['author'] ),
...
function pgscl_export_author_name( $author_id ) {
...
function pgscl_rest_export( WP_REST_Request $req ) {
	global $wpdb;

	$max = (int) apply_filters( 'pgscl_export_max', PGSCL_EXPORT_MAX );
	...
	if ( '' !== $ids_raw ) {
		...
		if ( $ids ) {
			...
			$db = pgscl_filter_readable( (array) $db );
	...
	$rows = $wpdb->get_results( $wpdb->prepare( $sql, $params ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $sql is a static string with %d/%s placeholders; values bound via prepare().

	// Exclude private pages the current user may not read from the export.
	$rows = pgscl_filter_readable( (array) $rows );
 ...
 * pgscl_csv_line() handles all quoting/escaping (commas, quotes, newlines) per RFC 4180.
 */
function pgscl_stream_csv( array $rows ) {
	...
	echo "\xEF\xBB\xBF"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- binary BOM for a CSV download, not HTML.

	// Header row is derived from the same map as the data rows.
	$header_written = false;

	foreach ( $rows as $r ) {
		$cols = pgscl_export_columns( pgscl_shape_row( $r ) );
		if ( ! $header_written ) {
			echo pgscl_csv_line( array_keys( $cols ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSV field output for a file download, escaped by pgscl_csv_line().
			$header_written = true;
		}
		// Guard against CSV/formula injection in spreadsheet apps.
		echo pgscl_csv_line( array_map( 'pgscl_csv_safe_cell', array_values( $cols ) ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSV field output for a file download, escaped by pgscl_csv_line().
	}

	// Empty result set: still emit a header row so the file isn't blank.
	if ( ! $header_written ) {
		$cols = pgscl_export_columns( array(
			'id' => '', 'title' => '', 'slug' => '', 'url' => '', 'status' => '',
			'author' => 0, 'parent' => '', 'parentTitle' => '', 'templateTerms' => array(),
			'elementor' => false, 'modified' => '',
		) );
		echo pgscl_csv_line( array_keys( $cols ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- CSV header output for a file download, escaped by pgscl_csv_line().
	}
...
/**
 * Format one CSV record (RFC 4180) as a string, terminated with CRLF.
 * Replaces fputcsv()/php://output so no direct filesystem handles are used.
 */
function pgscl_csv_line( array $fields ) {
	$out = array();
	foreach ( $fields as $field ) {
		$field = (string) $field;
		// Quote if the field contains comma, quote, CR or LF; double embedded quotes.
		if ( preg_match( '/[",\r\n]/', $field ) ) {
 ...
 */
function pgscl_csv_safe_cell( $value ) {
	$value = (string) $value;
...
function pgscl_rest_stats( WP_REST_Request $req ) {
	...
	$filtered = (int) $wpdb->get_var( $params ? $wpdb->prepare( $sql, $params ) : $sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $sql interpolates only $wpdb->posts and a validated {$join}; user values bound via prepare().
...
function pgscl_rest_tree( WP_REST_Request $req ) {
	global $wpdb;
	...
	$children = pgscl_filter_readable( $children );
	if ( ! $children ) {
		return rest_ensure_response( array() );
	}
...
function pgscl_rest_search( WP_REST_Request $req ) {
	...
	if ( 'children' === sanitize_key( (string) $req->get_param( 'scope' ) ) ) {
		return pgscl_search_children_of_matches( $req );
	}
	// ─────────────────────────────────────────────────────────────────────
	...
	$boolean = pgscl_to_boolean_query( $q );
	$ids     = array();

	$has_title_ft   = pgscl_index_exists( $wpdb->posts, 'pgscl_title_ft' );
	$has_content_ft = pgscl_index_exists( $wpdb->posts, 'pgscl_content_ft' );
	$has_meta_ft    = pgscl_index_exists( $wpdb->postmeta, 'pgscl_meta_value_ft' );
	...
	if ( ! $ids ) {
		...
		return rest_ensure_response( array( 'rows' => array_map( 'pgscl_shape_row', $like ) ) );
	}
...
function pgscl_to_boolean_query( $q ) {
 ...
 * --------------------------------------------------------------------------
 * ADDITIVE. Does not modify pgscl_rest_search()'s existing tiers. A 3-line
 * guard at the top of pgscl_rest_search() hands off here when scope=children.
 *
 * Logic:
 ...
 *      pgscl_rest_search() — so every tier (ID / slug / URL / FULLTEXT / LIKE)
 ...
 * ======================================================================== */
function pgscl_search_children_of_matches( WP_REST_Request $req ) {
	global $wpdb;
	...
	$parent_resp = pgscl_rest_search( $parent_req );
	...
	// Drop private children the current user cannot read.
	$children = pgscl_filter_readable( (array) $children );
	...
	$parents = pgscl_filter_readable( (array) $parents );
...
function pgscl_seo_keys() {
	switch ( pgscl_seo_plugin() ) {
		case 'rankmath':
			return array(
...
function pgscl_get_seo_data( $id ) {
	$plugin = pgscl_seo_plugin();
	$keys   = pgscl_seo_keys();
	...
	if ( 'rankmath' === $plugin ) {
		...
		if ( is_array( $r ) && ! empty( $r ) ) {
			...
			$data['robots']      = pgscl_rankmath_default_robots( get_post_type( $id ) );
 ...
 */
function pgscl_rankmath_default_robots( $post_type ) {
...
function pgscl_save_seo_data( $id, array $seo ) {
	$plugin = pgscl_seo_plugin();
	$keys   = pgscl_seo_keys();
	if ( ! $plugin ) {
		return;
 ...
 */
function pgscl_rest_edit_data( WP_REST_Request $req ) {
	$id = (int) $req['id'];
...
function pgscl_rest_quick_edit( WP_REST_Request $req ) {
	$id = (int) $req['id'];
	if ( ! current_user_can( 'edit_post', $id ) ) {
		return new WP_Error( 'pgscl_forbidden', __( 'Not allowed to edit this page.', 'pagescale' ), array( 'status' => 403 ) );
	}
	...
	if ( isset( $body['status'] ) && in_array( $body['status'], PGSCL_STATUSES, true ) ) {
	...
	if ( isset( $body['seo'] ) && is_array( $body['seo'] ) ) {
		pgscl_save_seo_data( $id, $body['seo'] );
	}

	$post = get_post( $id, ARRAY_A );
	return rest_ensure_response( pgscl_shape_row( $post ) );
}
...
function pgscl_rest_action( WP_REST_Request $req ) {
	$body   = $req->get_json_params();
	$action = sanitize_key( $body['action'] ?? '' );
	$id     = absint( $body['id'] ?? 0 );

	if ( ! $id || ! get_post( $id ) ) {
		return new WP_Error( 'pgscl_bad_request', __( 'Invalid page.', 'pagescale' ), array( 'status' => 400 ) );
	}

	switch ( $action ) {
		case 'duplicate':
			// Per-page check: must be able to edit the specific source page.
			if ( ! current_user_can( 'edit_post', $id ) ) {
				return new WP_Error( 'pgscl_forbidden', __( 'Not allowed to duplicate this page.', 'pagescale' ), array( 'status' => 403 ) );
			}
			$new = pgscl_duplicate_page( $id );
			if ( is_wp_error( $new ) ) {
				return new WP_Error( 'pgscl_dup_failed', $new->get_error_message(), array( 'status' => 400 ) );
			}
	...
	return new WP_Error( 'pgscl_unknown_action', __( 'Unknown action.', 'pagescale' ), array( 'status' => 400 ) );
}

function pgscl_duplicate_page( $id ) {
	$src = get_post( $id );
	if ( ! $src ) {
...
function pgscl_rest_duplicates( WP_REST_Request $req ) {
	global $wpdb;
	...
	if ( ! in_array( $mode, array( 'title', 'slug_exact', 'slug_similar' ), true ) ) {
		return new WP_Error( 'pgscl_bad_mode', 'mode must be title, slug_exact or slug_similar', array( 'status' => 400 ) );
	}
	...
	if ( 'title' === $mode ) {
		 ...
		 */
		// Build one placeholder per base ( post_name LIKE %s ), then a single
		...
		$sql = "SELECT ID, post_title, post_name, post_status, post_author, post_parent, post_date, post_modified
			 ...
			 ORDER BY post_name ASC, ID ASC
			 LIMIT %d";
		$rows = $wpdb->get_results( $wpdb->prepare( $sql, $params ), ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared,PluginCheck.Security.DirectDB.UnescapedDBParameter -- $sql contains only literal 'post_name LIKE %s' placeholders; all values bound via prepare( $params ).
	}

	$rows   = pgscl_filter_readable( $rows );
	$shaped = array_map( 'pgscl_shape_row', $rows );
...
add_action( 'save_post_page', 'pgscl_bust_total_cache' );
add_action( 'deleted_post', 'pgscl_bust_total_cache' );
...
if ( ! defined( 'PGSCL_USER_JOURNEY_TAX' ) ) {
	define( 'PGSCL_USER_JOURNEY_TAX', 'pgscl_user_journey' );
}
...
function pgscl_register_user_journey_tax() {
	register_taxonomy( PGSCL_USER_JOURNEY_TAX, 'page', array(
		...
		'labels'            => array(
			...
			'singular_name' => __( 'User Journey', 'pagescale' ),
			'menu_name'     => __( 'User Journey', 'pagescale' ),
		),
		'public'            => false,
	...
	if ( ! term_exists( 'yes', PGSCL_USER_JOURNEY_TAX ) ) {
		wp_insert_term( 'Yes', PGSCL_USER_JOURNEY_TAX, array( 'slug' => 'yes' ) );
	}
}
...
function pgscl_user_journey_ids() {
	...
	return array_map( 'intval', (array) apply_filters( 'pgscl_user_journey_ids', $ids ) );
}
...
function pgscl_get_app_js() {
	return <<<'PGSCL_JS'
	(function(){
	"use strict";
	var B = window.PGSCL_BOOT;
...
// phpcs:enable WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.DirectDatabaseQuery.SchemaChange, WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.PreparedSQLPlaceholders.UnfinishedPrepare
